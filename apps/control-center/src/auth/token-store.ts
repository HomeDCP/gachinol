 ...
 *
 * **웹 분기(T-W2-04, `apps/reporter` T-W2-01 동형 이식)**: `expo-secure-store`는 웹을 지원하지 않는다
 * (`ExpoSecureStore.web.ts`가 빈 객체 `{}`를 export — 호출하면 크래시). 게다가 웹 세션은 애초에 설계가
 * 다르다: `services/api` 웹 로그인 응답(`WebSessionResponse`)은 refresh 토큰 **원문을 바디에 담지 않는다**
 * — refresh는 HttpOnly 쿠키 전용으로 서버가 `Set-Cookie`(웹 세션 쿠키)로만 내려주고, JS는 그 값을
 * 읽을 수도 쓸 수도 없다(`services/api/src/auth/auth.controller.ts`의 `WebSessionResponse` 주석).
 ...
 *
 * `platformOS`는 테스트 DI용(기본값 = 런타임 `Platform.OS`) — `apps/reporter`와 동형 관례:
 * `jest.mock('react-native', ...)`으로 전역 Platform을 흔들지 않고도 웹 분기를 단위 테스트할 수 있게
 * 한다. 기존 `createTokenStore()` 무인자 호출부(`auth-context.tsx`)는 기본값 경로라 무변경으로 계속 동작한다.
 */
