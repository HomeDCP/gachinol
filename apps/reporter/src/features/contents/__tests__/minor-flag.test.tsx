 ...
 *     싣는다 — PATCH에서 키 생략='변경 없음'이므로 생략하면 서버의 fail-closed 무효화
 *     (true→false 시 확인 기록 삭제, shared dto.ts)에 도달할 수 없다.
 *  ④ 센터가 이미 동의를 확인한 콘텐츠에서 체크를 해제하면 "확인 기록 삭제" 경고가 보인다.
 *
...
const GUIDE_RE = /법정대리인 동의서를 준비/;
const WIPE_WARNING_RE = /동의 확인 기록이 함께 삭제/;
...
describe('초안 수정 화면 — 프리필·명시적 전송·무효화 경고 (AC3·AC4)', () => {
  ...
  it('★ 센터 확인 완료 콘텐츠에서 체크를 해제하면 "확인 기록 삭제" 경고가 보인다 (AC4)', async () => {
    mockGetContentDetail.mockResolvedValue(
      detail({
        hasMinorSubject: true,
        minorConsentConfirmedByUserId: 'u-center',
        minorConsentConfirmedAt: '2026-08-25T00:00:00.000Z',
      } as Partial<Content>),
    );
    const utils = await renderEdit();

    await utils.findByText(CHECKBOX_LABEL);
    expect(utils.queryByText(WIPE_WARNING_RE)).toBeNull();

    await fireEvent.press(utils.getByText(CHECKBOX_LABEL));
    expect(utils.getByText(WIPE_WARNING_RE)).toBeTruthy();
  });

  it('확인 전 콘텐츠에서는 해제해도 그 경고가 뜨지 않는다 — 지울 기록이 없다', async () => {
    mockGetContentDetail.mockResolvedValue(detail({ hasMinorSubject: true }));
    const utils = await renderEdit();
