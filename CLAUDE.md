...
- **실제 개발은 서브에이전트(AI 팀원)가 한다.** 팀 정의는 [`.claude/agents/`](.claude/agents/README.md) — 6롤
  (`implementer`·`verifier`·`wiring-auditor`·`ops-prober`·`scribe`·`planner`).
  ⚠️ **에이전트 정의는 세션 시작 시점에 로드된다** — 세션 중간에 만든 정의는 그 세션에서 호출할 수 없다.
...
⭐ **③의 요청 시점은 정본이 있다 — [QUEUE.md](docs/plan/exec/QUEUE.md) "사용자 실행 항목" 절.**
각 항목이 **어느 행을 막는지**와 **언제 요청해야 하는지**가 표로 못박혀 있다.
조율자는 그 시점에 도달하면 **그날 일간보고서 ①에 올려 요청한다** —
미리 몰아서 묻지 않고(결정 피로), 닥쳐서 묻지도 않는다(대기가 착수를 막는다).
**리드타임이 있는 항목은 그것이 막는 작업보다 리드타임만큼 앞서 요청한다.**
...
- **⑩머지는 사람.** 예외 없다 — **main 머지 = 제온 프로덕션 자동 배포**이기 때문이다.
  ⭐ **그리고 산문으로 두지 않는다**(2026-08-31 사용자 채택): 자동 커밋·푸시는 `feat/*`까지만 허용하고,
  **프로덕션 배포는 GitHub Environment의 required reviewer로 서버측 강제**한다. 지금은 로컬 push와
  프로덕션 사이에 **아무 장치가 없다**(`deploy-web.yml`에 `environment:` 0건). 설정은 **사용자 실행**.
...
| 웹앱             | 위치                  | 사용자         | 핵심 기능                                                                                                                                                                             |
| ---------------- | --------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **기자 웹**      | `apps/reporter`       | 12개 지사 기자 | 촬영(웹 `<input capture>`, ⚠️ **2026-09-19 대장 #230 확정 — 이 경로가 iOS Safari에서 구조적으로 360×480 저화질만 낸다**(WebKit 버그 197216, 1차 출처 확인. `expo-image-picker` 웹 구현이 `quality`/`videoQuality` 옵션을 읽지 않아 라이브러리로도 못 고친다). 2026-09-20 화면 안내는 갤러리 경로(휴대폰 기본 카메라 앱으로 먼저 찍어 올리기)를 주 버튼으로 승격했고(1차 수리, PR #108 배포 완료), **근본 수리로 `getUserMedia`+`MediaRecorder` 자체 촬영기 신설이 대장 #234로 채번돼 착수 게이트(70초 실기 검증) 통과·설계 결정 대기 중**이다) → 장면별 자막/내용 기입 → 분류 → 업로드. 메인서버가 최종편집해 반환 → 기자가 **저화질 미리보기** 확인 후 **승인** → 송출(카톡채널=지역방송국은 **반자동 게시**: 백엔드 게시자산 준비 → 담당자 관리자앱 게시. infra §5-1) |
| **센터 관제 웹** | `apps/control-center` | 제주방송센터   | 지사 업로드물 **자동 분석·저장** → 화면+텍스트 분석 → **매주 콘텐츠 추천** → 승인 시 즉시 송출 / 수정사항 입력 시 반영해 재생성. 라이브 관제 + 채널별 댓글 프롬프터 (웹 전환으로 데스크톱 콘솔 겸용 — 舊 "웹 콘솔 병행" 미정 항목 해소). **기자 업로드 완료·복구 대행**(기자가 올리다 막힌 업로드를 센터가 완료 또는 실패 처리해 재시도를 열어 주는 운영 권한 — 관리자 역할이며 admin 단독이면 실무가 한쪽으로 몰린다, 사용자 결정 2026-09-15 — **소유 기자 본인도 자기 업로드를 복구할 수 있다**(사용자 결정 2026-09-15); ⚠️ 촬영·콘텐츠 생성 자체는 여전히 기자 몫). 서버 API·관제 웹 UI 둘 다 배선 완료(대장 #216·#224, 브랜치 `feat/upload-stall-recovery`) — **머지·배포 전**(#224와 같은 PR 대기 상태). ⚠️ **기자 웹에는 아직 복구 수단이 없다**(대장 #225) |
...
- **🔧 제품 모델 갱신 — 송출의 정의 재확정 (2026-09-20 사용자 결정, 대장 #230 조사 연장 중 촉발)**: 사용자가
  남긴 결정 원문 두 문단을 그대로 인용한다(규율 13 — 향후 이 항목이 다시 갱신돼도 원문은 지우지 않는다).

  > 우리는 방송국이고, 방송국에서 기자가 보낸 영상을 편집해주고, 송출까지는 한다. 실제 송출채널은 카카오톡
  > 페이지, 유튜브 등 다른 미디어플랫폼에 실어서 내보낼 것이다. 따라서 우리가 걱정해야하는 비용은 기자가
  > 찍어서 보내오고, 우리가 처리한 다음 유뷰브 및 카카오톡에 업로드하기까지이다. 업로드 해두면 시청자들이
  > 알아서 보도록 할 것이다. 지금은 시청앱이 서버에서 바로 나가도록 구현되어 있지만, 추후엔 유튜브 임베디드나
  > 다른 형태를 활용해서 서버비용을 우리가 쓰지 않는 형태로 바꿀 것이다. 이와 관련해서는 기술 및 법률검토가
  > 필요하고, 이런 부분은 회사를 만들어 투자를 받으면서 해결 될 부분으로 비춰진다.

  > 지금 고화질 마스터 필요하고, 프리뷰 필요하지만, 720p 렌디션은 필요하지 않아보인다. 유튜브에 올려두면
  > 대역폭에 맞춰 적당히 낮춰서 스트리밍 해주니 필요하지 않고, 카카오톡 페이지도 같은 기능을 지원한다.
  > 우리가 MVP만들면서 간단히 돌아가는 걸 확인하는 용도로 필요하니 사용은 하되(투자자에게 시연이 필요하니까)
  > 추후 정식 서비스에 들어갈 때에는 기자의 촬영본(고화질 원본)을 제온서버가 받고, 제온서버가 편집한
  > 다음(고화질 편집본) 이 고화질 편집본을 기자에게 공유할 때만 프리뷰로 화질 낮춰서 보내고, 고화질
  > 편집본이 그대로 송출되어야한다. 송출은 유튜브, 카카오페이지 업로드를 의미한다.

  핵심 세 가지를 정본 문언으로 정착한다: ⑴ **송출 = 외부 플랫폼(YouTube·카카오) 업로드**이며 우리가 처리한
  고화질 편집본이 그대로 올라간다 ⑵ **프리뷰는 기자 공유용으로만** 저화질(승인 전 확인 목적) ⑶ **720p
  렌디션은 지금의 MVP 시연(투자자 시연 포함) 기간 한정**이고, 정식 서비스에서는 유튜브·카카오 자체가
  대역폭에 맞춰 스트리밍 화질을 조절하므로 불필요해진다. **구독자 웹이 서버에서 직접 스트리밍하는 지금
  구조를 폐기한다는 결정은 아니다** — 서버 비용을 우리가 쓰지 않는 형태(유튜브 임베디드 등)로의 전환은
  기술·법률 검토가 선행돼야 하는 별도 축이고, 사용자 원문대로 투자 유치 이후로 전망되는 사안이다.
  ⚠️ **이 결정 과정에서 구조적 결함이 드러나 신규 채번됐다(대장 #232)**: `auto_edit`(자동편집) 산출물
  `edited_master`가 배포 렌디션과 **같은 규격**(720p/2500kbps)으로 나오도록 설계돼 있어(2026-08-20 Phase 1
  결정 — 아래 "자동편집 Phase 1 완료" 항목 참조, 재편집 속도가 근거였고 당시엔 외부 송출 화질 축이
  제기되지 않았다) 외부 플랫폼에 올릴 고화질 원천 자체가 파이프라인에 없다. 처방은 설계 결정 대기(planner
  발주됨, PIVOT-PLAN 대장 #232는 등재만·미확정). 관련: 대장 #233(서명 프리뷰 URL이 CDN 캐시 키를 시청자별로
  갈라놓는 구조, 같은 조사 연장 중 관찰)·대장 #234(대장 #230의 근본 수리 — `getUserMedia`+`MediaRecorder`
  자체 촬영기, 착수 게이트 통과·설계 대기).
...
- **인터넷신문(언론사) 등록 여부 — 내부 검토 중(2026-09-01)**: 07 §3-5(언론중재법)가 발주한 항목.
  등록 시 언론 지위·정정보도 의무가 생기는 대신 "언론 보도 목적" 초상권 침해 면책 법리 원용 폭이
  넓어지는 트레이드오프. **사용자가 내부 검토를 시작**했고, 외부 법률자문 트레이드오프 분석은 아직
  의뢰 전이다. ⚠️ **별개 축과 혼동 금지**: 이 결정은 §3-6 초상권(인격권) 축에 관한 것이고,
  개인정보보호법 제22조 아동 법정대리인 동의(07 §3-3)에 미치는 영향은 **불확실하며 확정된 바 없다**
  — "등록하면 아동 동의 의무가 자동으로 풀린다"는 아직 검증 대상 가설이다(법률자문에 명시적으로
  물어야 할 질문, 07 §3-3·§3-5 양쪽에 상세 기록). 촬영은 되돌릴 수 없으므로 이 답이 나오기 전에는
  "등록했으니 동의서 불요"를 전제로 촬영을 재개하지 않는다.
