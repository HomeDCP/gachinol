...
| 코드 태스크 | **46건 중 26 완료 · 20 미착수 (57%)** (2026-08-28 재실측 + 오검출 전수 스윕) — ⚠️ grep 원값은 **34**(E2 §C 행 48 기준, 명령은 §5): 언급-오검출 **6건**(기지 3: T-W1-11b·T-W2-17·T-W4-02 §7-1 + 신규 3: **T-W2-05·T-W2-02·T-W1-07a** §7-0) + **모수 밖 신설 2건**(T-W2-19·T-W2-35 — E2에 행은 있으나 §0 총계 "코드 46"에 미편입) 포함. 舊 원값 32는 T-W2-19 행이 열거에 없던 값이고, 舊 "실제 29/46(63%)"는 신규 오검출 3건을 실구현으로 오계상한 값이다. 모수 밖 완료 3건 — T-W2-19·T-W2-35·**T-W2-36(E2 행 자체가 없음)** — 은 **대장 기준으로 셀 것**(#71·#147·#166 비고) | 아래 §5 |

**착수 전에 이 네 줄을 먼저 돌린다.** 값이 다르면 이 문서가 stale인 것이므로, 문서를 믿지 말고 실측을 믿는다.

## 2. 지금 갈 수 있는 것 (미착수 20건 중 8건)
...
| T-W2-06 | 관제 앱 토큰 전환 + `theme.ts` 제거(17파일 XL) | 기계적 반복(분할 예외②) — 패턴 선례는 구독자 T-W1-02 **1회뿐**(舊 "기자 T-W2-05로도 검증"은 오검출이 만든 허상, §7-0). T-W1-01 의존 충족. ⚠️ 착수 시 대장 **#68**(lineHeight 하드코딩) 사이징 재판정 동반 |
| T-W1-07a | 구독자 시청 계측 소비 + 큰 자막 모드 토글(03§A-3/§A-6) | ⚠️ **오검출 정정 재편입**(§7-0). 41569e9 본문 자신이 "소비 트랙은 T-W1-07a **잔여**"라 명시 |
...
⚠️ 원값 34 = 실구현 28 + 언급-오검출 6(§7-0·§7-1). 실구현 28에서 **모수 밖 신설 2건**(T-W2-19·T-W2-35 —
행 48 > 선언 46의 차이가 정확히 이 둘이다. `34d1903^` 시점 행수 46으로 실증)을 빼면 **26/46**.
...
- **언급-오검출은 3건이 아니라 6건이었다** — 신규 3건(T-W2-05·T-W2-02·T-W1-07a)은 각각 딴 태스크의
