...
- **대장 #217·#218·#220·#221**(2026-09-13 "업로드 경로 복구" 슬라이스가 드러낸 잔여 4건, 전부
  ...
  슬라이스에서 2회 더**(implementer·조율자 각 1회, 위 갱신 절 참조) 재현하며 실패 스위트를 특정했다
  (`all-exceptions.filter.spec.ts`, 원인은 jest worker SIGSEGV — 테스트 코드 결함이 아니라 스위트
  실행 자체의 크래시). 원인은 여전히 미상이라 해소 아님. 상세는 PIVOT-PLAN 대장 해당 행.
