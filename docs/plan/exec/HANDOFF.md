...
## ⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 지금의 다음 1건 (2026-09-21, scribe 갱신 — 대장 #232 태스크① 배포 완료, 태스크②~⑤·사용자 결정 4건 정리 — 이번부터 문서 최상단에 둔다 / scribe 2차(2026-09-22) — 태스크② 완료·PR #112 대기, 다음 1건=#236, 사용자 대기 8건, 오디오 실측 추가, 함정 절 재정렬 — 같은 절을 갱신한다, 규율 13 / scribe 3차(2026-09-22) — 1080p 마스터 실증 완료(대장 #232)·대장 #236 착수 게이트 해제·#239 채번 / scribe 4차(2026-09-22) — 사용자 결정 5건 확정·#240·#241 채번·PR #112·#113 머지 완료로 #139 해소·#238 차단 해제·#236 구현 중 / scribe 5차(2026-09-23) — 대장 #242 채번(ffmpeg 바이너리 환경차), PR #112~#115 머지 완료로 #236 해소, #240·#241 구현완료·PR #116 CI 수리 중, 함정 2026-09-23 신설)
...
| ③ | 송출 메시지가 마스터를 싣는다 — `distribution-producer.buildMessage`가 `edited_master` presign(대장 #236) | L3 | ①②, 대장 #211(해소됨) | ✅ **해소(2026-09-23)** — PR #112~#115 전부 머지·배포 완료(main `9a6a0e9`, 제온 실물 api·media-worker sha-9a6a0e9 확인) |
| ④ | 재생성 소스 폴백(대장 #237) | L3 | ① | 미착수 |
| ⑤ | 720p 제거 스위치 정착(대장 #238) | L3 | ①② | **조건 확정, 착수 가능(발동은 수동)** — 사용자 결정 2 확정 |

**진행 반영(2026-09-23, 조율자) — PR #112~#115 전부 머지·배포 완료.** main `9a6a0e9`, 제온 실물
api·media-worker 이미지 둘 다 `sha-9a6a0e9`, `deploy` 잡 실제 실행(스킵 아님, 조율자 실측 인용 —
scribe는 제온 SSH 금지라 독자 재현 못함). ⇒ **대장 #236 해소**(위 표). **대장 #240·#241은 구현·
검증 완료 — PR #116으로 제출, CI 수리 중**(회전 픽스처 문제, 아래 참조). 커밋 `dcbbbfb`.

**⭐⭐ 신규 채번 — 대장 #242(ffmpeg 바이너리가 환경마다 다르다, 조율자 실측 2026-09-23).** 오늘
찾은 것 중 구조적으로 가장 중요하다(ROOT-CAUSE "검사가 닿지 않는 축"의 전형). 메이저 버전이
셋 다 다르다(조율자 실측 인용 — scribe는 `services/**`·제온 SSH·CI 로그 비접근): 프로덕션(제온
컨테이너) ffmpeg **5.1.9**(Debian bookworm, `ffmpeg-static/ffmpeg`가 `/usr/bin/ffmpeg` 심볼릭
링크로 교체됨) / 로컬 개발(macOS) `ffmpeg-static` npm **6.0** / CI(GitHub Actions Linux)
`ffmpeg-static` npm **7.0.2-static**(johnvansickle, PR #116 재실행 CI 로그에서 확인). **방향이
문제다 — 프로덕션이 셋 중 가장 낮은 버전**이라 "테스트 통과 = 프로덕션 동작"이 성립하지 않는다.
실제 사례: `-display_rotation`(ffmpeg 6.0+ 옵션)이 로컬·CI에는 있고 프로덕션 5.1.9에는 없다 —
픽스처 생성에만 쓰고 프로덕션 경로에 넣지 않아 사고가 안 났을 뿐, 프로덕션 코드에 넣었다면
배포 후에야 터졌을 것이다. 또한 **ffmpeg 7.x에서 deprecated `rotate` 태그를 Display Matrix로
바꿔 주던 동작이 빠진 것**이 PR #116 CI 실패의 직접 원인이었다(방법 A 실패 → 방법 B 폴백이 받아
통과 — 폴백이 없었으면 CI는 계속 빨갰다). **심각도는 올리지 않는다** — 결함이 아니라 구조적
검사 공백이고, 지금 프로덕션 동작은 정상임이 실측으로 확인됐다. 처방은 미정(조율자·planner
판정 대상). 상세는 PIVOT-PLAN 대장 #242 행.
...
- ⭐ **신규(2026-09-22 채번, scribe)**: **#236**(태스크③, 위 표 — ✅ 해소) ·
  **#237**(태스크④, 재생성 소스 폴백, 미착수) · **#238**(태스크⑤, 조건 확정·착수 가능·발동은
  수동) · **#239**(세로 썸네일 프로파일 비대칭 — 조율자 관찰) · **#240**(세로 마스터가 Shorts
  네이티브 규격 미달 — 구현·검증 완료, PR #116 CI 수리 중) · **#241**(마스터 오디오 규격
  상향 — 구현·검증 완료, PR #116 CI 수리 중).
- ⭐ **신규(2026-09-23 채번, scribe)**: **#242**(ffmpeg 바이너리 환경차 — 조율자 실측,
  처방 미정, 위 참조).
...
grep -n "^| 139 \|^| 211 \|^| 230 \|^| 232 \|^| 233 \|^| 234 \|^| 235 \|^| 236 \|^| 237 \|^| 238 \|^| 239 \|^| 240 \|^| 241 \|^| 242 " docs/plan/PIVOT-PLAN.md
node infra/scripts/daejang-recheck.mjs --only 139,211,230,232,233,234,235,236,237,238,239,240,241,242
```

**이 세션(2026-09-23)이 남긴 함정 5건** — 전문은 아래 "## 함정 2026-09-23 — 세션(대장
#240·#241·#242)이 남긴 함정" 절(함정 시리즈 최상단) 참조. 요지만:
1. ⭐ 픽스처가 검사를 무력화하는 패턴이 한 세션에 네 번 나왔다 — 픽스처는 자기를 검증해야
   한다(생성 후 전제를 probe로 확인, 못 만들면 skip이 아니라 throw).
2. 환경이 셋이면 로컬 초록은 아무것도 보장하지 않는다(대장 #242 — ffmpeg 5.1.9/6.0/7.0.2로
   프로덕션이 가장 낮다).
3. 채번은 위임 전에 끝내고 번호를 위임문에 박아라 — 안 그러면 구현자가 번호를 지어낸다.
4. `sed -i`가 auto mode 분류기에 차단된다 — `Path.read_text`/`write_text`로 우회.
5. `pnpm run test:scripts | tail`은 exit code를 삼킨다 — 리다이렉트로 받아 읽을 것.

⚠️ **정본 갱신 필요(CLAUDE.md §11, 이번 발주 범위 아님)**: media-worker 테스트 실측이
**44(작업 전)→52(작업 후)**인데 CLAUDE.md §11은 **35**로 적고 있다(조율자 실측 인용 —
scribe는 `services/**` 실행 없이 독자 재현 못함). api는 통합 상태 **1067**.

**직전(2026-09-22) 세션이 남긴 함정 4건**은 아래 "## 함정 2026-09-22" 절 참조(이번 세션
함정 절 바로 아래로 이동).
...
## 함정 2026-09-23 — 세션(대장 #240·#241·#242)이 남긴 함정

1. 픽스처가 검사를 무력화하는 패턴이 한 세션에 네 번 나왔다. 미디어 픽스처 320x240이 min(ih,H)를
   항등으로 만들어 값을 뭘로 바꿔도 초록이었던 것(태스크①에서 수리) / 픽스처가 전부 가로라
   방향 축 결함(대장 #240)을 아무도 못 잡은 것 / 오디오 픽스처가 사인파라 256k를 요청해도 실측
   약 142kbps로 128k와 구분이 안 돼 anoisesrc로 교체한 것 / 회전 픽스처가 플랫폼에 따라 메타
   없이 만들어져 "회전을 검증한다"는 이름만 달고 아무것도 검증하지 않은 것. 넷 다 같은 형태다 —
   테스트는 초록인데 검증은 없다. 그래서 픽스처는 자기를 검증해야 한다: 생성 후 전제를 probe로
   확인하고, 못 만들면 skip이 아니라 throw한다.
2. 환경이 셋이면 로컬 초록은 아무것도 보장하지 않는다(대장 #242). ffmpeg 메이저 버전이 프로덕션
   5.1.9 · 로컬 macOS 6.0 · CI Linux 7.0.2-static로 셋 다 달랐다 — 로컬 macOS 통과, CI Linux
   실패, 프로덕션은 실측상 정상, 세 결과가 다 달랐다. 프로덕션이 셋 중 가장 낮은 버전이라 "테스트
   통과 = 프로덕션 동작"이 성립하지 않는다. `-display_rotation`(ffmpeg 6.0+ 옵션)이 실제 사례다 —
   프로덕션 경로에 썼다면 배포 후에야 드러났을 것이다. ⇒ 신기능을 프로덕션 경로에 쓰기 전에 제온
   ffmpeg에 그 옵션이 있는지 먼저 확인할 것(재현): `ssh xeon 'docker exec
   gachinol-prod-media-worker-1 ffmpeg -hide_banner -h full 2>&1 | grep -c <옵션>'`.
3. 위임문에 "채번하겠다"고만 쓰고 번호를 안 주면 구현자가 번호를 지어낸다. 이번에 구현자가 존재하지
   않는 대장 #244를 코드 주석 7곳에 넣었다(조율자 정정 발주). ⇒ 채번은 위임 전에 끝내고 번호를
   위임문에 박는다.
4. `sed -i`가 auto mode 분류기에 차단된다("Irreversible Local Destruction"). 검증자가 python3의
   `Path.read_text`/`write_text`로 우회했다. 뮤테이션 작업 위임 시 미리 알려 두면 한 번의 실패를
   아낀다.
5. `pnpm run test:scripts | tail`은 exit code를 삼킨다 — `$?`가 `tail`의 코드다. 리다이렉트로
   받아 읽어야 한다(`> /tmp/x.log 2>&1; echo "EXIT=$?"`). 검증자가 실제로 밟았다.
6. `guard.sh`는 셸 명령 텍스트에 `.env` 문자열이 있으면 산문이라도 막는다(경로 판정이 아니라
   문자열 매칭). 조율자 실측: `echo "… .env …"` → 차단 / `echo "테스트다.** 볼드"` → 통과. ⇒
   `gh pr comment`·커밋 메시지 heredoc에 `.env`를 쓰지 말고 "환경변수 파일"처럼 바꿔 쓴다.
   훅 자체는 옳게 동작하는 것이며(보안 규율은 예외 없다) 우회 대상이 아니라 표현을 바꿀 대상이다.
   ⚠️ 이 함정은 한 번 잘못 진단됐다 — 2026-09-23 scribe가 원인을 "마침표 뒤 볼드마커(`.**`)"로
   보고했으나 조율자 재실험에서 반증됐다. 볼드는 무관하다. 진단도 기록치와 같아서 재실행으로
   검증해야 한다(규율 1).
