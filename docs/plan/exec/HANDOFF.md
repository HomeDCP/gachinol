...
> ⚠️ 아래 §1·§2는 2026-08-30 재판정 시점 값이며, **2026-08-31 전수 정독으로 순서가 바뀌었다**
> — 착수 판정은 QUEUE.md를 보라.
...
**착수 순서는 반드시 [QUEUE.md](QUEUE.md)로 판정하라.** 아래 §1·§2는 08-30 시점 값이라 stale이다.

### 다음 1건
**QUEUE 0-1 · 대장 #182 — `daejang-recheck` 자기검사·정정.**
舊 QUEUE 0-1(대장 #186 빌드 스탬프 + 배포 후 SHA 대조)은 **2026-08-31 해소**됐다(커밋 `e9f47f1`·
PR #81 — 게이트② 뮤테이션 3건 red 확인, PR은 이 시점 오픈·CI 3종 pass·머지는 사람 대기). 행을
제거하고 나머지를 한 칸씩 당겨 QUEUE 0-1은 이제 **대장 #182**(대장 판정 도구 자체가 대장 아닌
행까지 세는 결함)를 가리킨다. 선행·차단자 없음.
