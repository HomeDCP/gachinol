...
## ⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 지금의 다음 1건 (2026-09-21, scribe 갱신 — 대장 #232 태스크① 배포 완료, 태스크②~⑤·사용자 결정 4건 정리 — 이번부터 문서 최상단에 둔다 / scribe 2차(2026-09-22) — 태스크② 완료·PR #112 대기, 다음 1건=#236, 사용자 대기 8건, 오디오 실측 추가, 함정 절 재정렬 — 같은 절을 갱신한다, 규율 13)
...
독자 재현 못함)**:
- **대장 #232 태스크② 완료 — PR #112 머지 대기.** 브랜치 `feat/asset-selector-unification`,
  ...
  ⚠️ **대장 #139는 머지 후에 해소로 표기한다 — 지금은 "구현 중"이다**(scribe가 이전 세션에서
  이미 그렇게 적어 뒀다, PIVOT-PLAN 대장 #139 행 참조).
...
⚠️ **단, 1080p 마스터 실증이 아직 없다**(사용자 고화질 업로드 대기 — 위 "오늘(2026-09-20~21)
완료" 블록의 재현 명령 참조). **이 실증이 ③의 착수 게이트다** — 검증되지 않은 자산을 외부
플랫폼(YouTube·카카오)에 실어 보내는 순서가 되면 안 된다.

| 순 | 내용 | 사다리 | 선행 | 상태 |
|---|---|---|---|---|
| ① | 송출 마스터를 렌디션에서 분리 | — | — | ✅ **완료**(PR #110, `83ddebf`, 배포됨) |
| ② | 목적별 자산 셀렉터 단일화(대장 #139) | L2 | 없음 | ✅ **구현 완료 — PR #112 머지 대기**(`8fde09e`) |
| ③ | 송출 메시지가 마스터를 싣는다 — `distribution-producer.buildMessage`가 `edited_master` presign(대장 #236) | L3 | ①②, 대장 #211(해소됨) | **미착수 — 착수 게이트: 1080p 마스터 실증**(사용자 고화질 업로드 대기) |
| ④ | 재생성 소스 폴백(대장 #237) | L3 | ① | 미착수 |
| ⑤ | 720p 제거 스위치 정착(대장 #238) | L3 | ①②, 사용자 결정 대기 ⓔ | **착수 불가**(선행 결정 미확보) |

**사용자 결정·요청 대기 8건**(scribe 2차 갱신, 2026-09-22 — 舊 4건에 조율자 요청 3건·오디오
채번 여부 1건을 더해 재편):
- ⓐ **고화질 원본 업로드**(태스크③=대장 #236의 착수 게이트 — 신규 콘텐츠 1건을 1080p 소스로
  업로드해야 `edited_master`가 1080p로 실증된다).
- ⓑ **프로덕션 DB 조회 권한** — 조율자가 권한 분류기(auto mode)에 막혀 직접 조회를 못하는
  사례가 반복되고 있다(리드타임 있는 항목, 조율자 판단 필요).
- ⓒ **Healthchecks.io grace 6h → 8h 상향**(조율자 실측 2026-09-22: schedule 런 78건 중 간격
  ...
  실측으로 확인됨).
- ⓓ **세로 1~3분 콘텐츠를 Shorts로 보낼지, 16:9로 변환해 일반 영상으로 보낼지** — 2024-10-15
  이후 "세로+3분 이하"는 YouTube가 **자동으로 Shorts로 분류**하고 Shorts는 **1080p 천장·1080p
  Premium 제외**다(출처·재현은 대장 #232 행). 화질 천장과 노출 경로가 갈리는 제품 결정.
- ⓔ **720p 렌디션 종료 조건**(태스크⑤=대장 #238의 차단자) — 값이 아니라 조건의 형태를 정해야 한다.
- ⓕ **기존 published 콘텐츠 재마스터링 여부** — ⚠️ **전이맵상 경로가 없다**: `grep -n "published:"
  packages/shared/src/content/workflow.ts` → `published: ['archived']`(재마스터링을 표현할 상태
  전이가 없다는 뜻, `archived: []`도 동일 파일에서 확인 가능).
- ⓖ **오디오 마스터 규격** — 아래 "오디오 실측 추가" 참조. 적용 대상이 송출 마스터(211행) 하나로
  좁혀졌으니 값(48kHz·스테레오 384k 등)을 정할지 여부만 남았다.
- ⓗ **오디오 축을 대장에 채번할지 여부** — 아래 실측이 처방을 요구하는 결함인지, 조사 기록으로만
  남길지 조율자 판단 필요(scribe는 판정하지 않는다).
...
- ⭐ **신규(2026-09-22 채번, scribe)**: **#236**(태스크③, 위 표) · **#237**(태스크④, 재생성
  소스 폴백) · **#238**(태스크⑤, 착수 불가 — 사용자 결정 ⓔ 대기).
...
grep -n "^| 139 \|^| 211 \|^| 230 \|^| 232 \|^| 233 \|^| 234 \|^| 235 \|^| 236 \|^| 237 \|^| 238 " docs/plan/PIVOT-PLAN.md
node infra/scripts/daejang-recheck.mjs --only 139,211,230,232,233,234,235,236,237,238
```
