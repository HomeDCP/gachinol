...
### 다음 1건
**QUEUE 0-1 · 대장 #179·#187 — 인터넷 경로 복구 + Quick Tunnel 재발 차단.** (2026-09-01 scribe 갱신)
舊 QUEUE 0-1(대장 #182 — `daejang-recheck` 자기검사·정정)은 **2026-09-01 완주**했다 — 표 물리 복구 +
기대값 문법 규정 + 표 무결성 전 조각 검사 + dist 제외. 게이트② 독립 검증 과정에서 도구 자신의 안전
결함 2건이 추가 발견·수리돼 **대장 #191**(`isSafe` 화이트리스트 우회 3종)·**#192**(splitCells 이스케이프와
grep BRE alternation 충돌)로 신규 채번·동시 해소됐다(재현: `node infra/scripts/daejang-recheck.mjs` →
표 무결성 0행·수동 판정 45건, 등재 시 56 대비 감소 — 2026-09-01 scribe 재실행). 행을 제거하고 나머지를
한 칸씩 당겨 QUEUE 0-1은 이제 **대장 #179·#187**(Quick Tunnel 호스트 불일치로 인터넷 경로 502 불통 +
재기동마다 100% 재발하는 구조)을 가리킨다. 선행 없음.
⚠️ **차단자: named tunnel(권장 선택지)은 외부 배선이라 사용자 승인이 필요하다.** QUEUE §C-1이
**"도달함"으로 갱신됐다** — 舊 0-1(#182) 완료가 곧 이 항목의 요청 시점이었다(오늘 일간보고서 ①에 게재).
⚠️ **§0-A 舊 문언("사용자가 '계획을 더 다듬는다'를 택해 개발 착수는 보류 중")은 더 이상 사실이 아니다**
— 2026-08-31 착수했고 대장 #186 슬라이스가 실제로 완주했다(위 해소 근거). #182도 그 뒤를 이어 완주했다.

### 사용자 대기 (일간보고서 ① 참조 — `docs/ops/daily/`)
① 제온 `DROP COLUMN` 마이그레이션 승인 · ② **Quick Tunnel 고정 방식(named tunnel 권장) — 요청 시점
도달(2026-09-01, QUEUE §C-1)**, 지금이 그 순간(0-1 착수 직전) · ③ 카카오 실 송출 범위 ·
④ GitHub Environment 승인 게이트 설정(원칙 채택됨, 배선은 사용자 실행)
...
| 항목 | 값 | 재현 |
|---|---|---|
| main | `8301eae` (PR #82 머지 — "Merge pull request #82 from HomeDCP/docs/slice-186-closeout") | `git fetch origin main && git log --oneline -1 origin/main` (§7-0 — 로컬 ref는 stale일 수 있다). ⚠️ 舊 기재 `325fd8c`(PR #81)는 PR #82 머지로 stale이 됐다 — 매 인계마다 재발하므로 착수 시 반드시 재실행할 것 |
| 열린 PR | 0 | `gh pr list --state open` |
| 대장 | **192행 / 대기 60**(2026-09-01 scribe 재실측 — #182 해소 반영, #191·#192 신규 채번·동시 해소 포함) | 아래 §5 |
| 코드 태스크 | **46건 중 27 완료 · 19 미착수 (59%)** — **불변**(2026-08-28 T-W2-02 머지 후 값과 동일, §5 명령 재실행으로 확인) — grep 원값 **34**(재실행 확인, 舊와 동일). 이번 슬라이스(대장 #182 — daejang-recheck 자기검사)는 **E2 §C 코드 태스크 목록 밖**(인프라 스크립트 결함 수리이며 태스크 ID를 커밋에 적지 않음)이라 이 모수에 편입되지 않는다 — 원값 불변이 진척 없음을 뜻하지 않는다(§5 마지막 경고와 동일 원칙) | 아래 §5 |
