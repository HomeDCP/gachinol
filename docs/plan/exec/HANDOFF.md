...
## ⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 지금의 다음 1건 (2026-09-21, scribe 갱신 — 대장 #232 태스크① 배포 완료, 태스크②~⑤·사용자 결정 4건 정리 — 이번부터 문서 최상단에 둔다)
...
다음 세션이 착수 전 확인할 것(재현: 신규 콘텐츠 1건 업로드 → auto_edit 완료 대기 → 그 세대
`edited_master` 자산을 서명 URL로 받아 `ffprobe -show_entries stream=height,bit_rate`로 대조).

**지금의 다음 1건 — 대장 #232 태스크②**(planner 설계안 5개 태스크 중 ①만 끝났다. 나머지는 전부
**미채번 — 착수 전 채번이 선행**, 규율 24-1):

| 순 | 내용 | 사다리 | 선행 |
|---|---|---|---|
| ② | **목적별 자산 셀렉터 단일화** — `feed.service.ts`·`public-media.service.ts`·`distribution-producer.service.ts` 3곳에 복제된 `renditionLabel === '720p'` 규칙을 `services/api/src/media/`의 단일 함수로. **대장 #139 해소 + 미등재 3번째 사본 등재**. D1-c(셀렉터 유일성 검사) 동반 | **L2** | 없음 |
| ③ | 송출 메시지가 마스터를 싣는다 — `distribution-producer.buildMessage`가 `edited_master` presign | L3 | ①②, 대장 #211(해소됨) |
| ④ | 재생성 소스 폴백 — 직전 세대 마스터가 구 프로파일(720p)이면 원본으로 폴백(`requireEditSource`) | L3 | ① |
| ⑤ | 720p 제거 스위치 정착 — auto_edit 렌디션 산출과 `enqueueTranscode`를 한 판정에 묶음 | L3 | ①②, **아래 결정 대기 2** |

②가 L2인 이유: ①이 배포된 지금 **사본 갈라짐이 실발생 결함이 됐다**(대장 #139의 착수 트리거가
*"렌디션 프로파일이 늘어날 때"*인데 그게 방금 일어났다). ⚠️ **scribe 관찰**: PIVOT-PLAN 대장 #232
상태 칸은 이 세션 시점에도 여전히 "설계 진행 중(처방 미확정)"으로만 적혀 있고 태스크① 배포 완료
사실이 그 칸에 반영돼 있지 않다(전체 설계가 여전히 진행 중이라는 서술 자체는 틀리지 않으나, 태스크
단위 진행 상황은 대장에서 보이지 않는다) — 대장 수정은 scribe 소관 밖이라 고치지 않고 보고만 한다.

**사용자 결정 대기 4건**(다음 QUEUE 편입·태스크③~⑤ 착수 전 조율자가 확보할 것):
1. ⭐ **세로 1~3분 콘텐츠를 Shorts로 보낼지, 16:9로 변환해 일반 영상으로 보낼지** — 2024-10-15
   이후 "세로+3분 이하"는 YouTube가 **자동으로 Shorts로 분류**하고 Shorts는 **1080p 천장·1080p
   Premium 제외**다(출처·재현은 대장 #232 행). 화질 천장과 노출 경로가 갈리는 제품 결정.
2. **720p 렌디션 종료 조건**(태스크⑤의 차단자) — 값이 아니라 조건의 형태를 정해야 한다.
3. **기존 published 콘텐츠 재마스터링 여부** — ⚠️ **전이맵상 경로가 없다**: `grep -n "published:"
   packages/shared/src/content/workflow.ts` → `published: ['archived']`(재마스터링을 표현할 상태
   전이가 없다는 뜻, `archived: []`도 동일 파일에서 확인 가능).
4. **오디오 규격** — 현재 하드코딩(`grep -n "audioBitrate" services/media-worker/src/ffmpeg.ts` →
   149·211행 `'128k'`, 242행 `'96k'`). YouTube 권장은 48kHz 모노 128k / 스테레오 384k.

**QUEUE 판정 대기 채번**(순서는 조율자 몫 — scribe는 나열만, `grep -n "^| 21[7]\|^| 22[2-9]\|^| 23[3-5] " docs/plan/PIVOT-PLAN.md`로 일괄 확인):
...
grep -n "^| 139 \|^| 211 \|^| 230 \|^| 232 \|^| 233 \|^| 234 \|^| 235 " docs/plan/PIVOT-PLAN.md
node infra/scripts/daejang-recheck.mjs --only 139,211,230,232,233,234,235
```

**이 세션이 남긴 함정 5건** — 전문은 아래 "## 7-000000000. 2026-09-20~21 세션이 남긴 함정" 절
(0-A 이후 함정 시리즈의 최상단, 9자리 0 — 바로 위 "7-00000000"과 혼동 주의) 참조. 요지만:
1. ⭐ **테스트 픽스처가 검사를 원천 차단한다** — 미디어 픽스처가 320×240이라 렌디션·마스터 프로파일을
   뭘로 바꿔도(720이든 1080이든) 항등이었다. `scale=-2:'min(ih,H)'`인 이상 소스가 작으면 어떤 H값도
   구분이 안 된다. 이번에 픽스처를 1920×1080으로 승격했다.
2. **워크스페이스 `--filter`가 `test:scripts`를 통째로 뺀다** — 루트 `pnpm test`는 3단계
   (`turbo run test && pnpm run test:scripts && turbo run test:web`)인데 `--filter`는 첫 단계만
   탄다. 코드 변경 위임문에는 `pnpm run test:scripts`를 반드시 함께 적을 것 — 안 그러면 CI에서야
   줄 수 게이트 위반이 드러난다.
3. **커밋 주체가 조율자로 바뀌었다** — `.claude/agents/implementer.md`는 "git 쓰기 금지, 커밋은
   조율자가 한다"고 명시하는데 위임문이 구현자에게 커밋까지 지시해 온 관행이 있었다. 2026-09-20부터
   조율자가 직접 커밋한다(예: `18902a6`은 조율자 커밋).
4. **번들 안의 한글은 `\uXXXX` 이스케이프다** — 배포 확인 시 원문 한글로 grep하면 전부 0이 나와
   오진한다.
5. ⚠️ **실측이 있는데 기록이 없으면 후속 판단이 왜곡된다** — #211은 2026-09-16에 실증됐으나 대장
   반영이 발주되지 않아, 2026-09-20 planner가 리포만 읽고 **"고화질 원본 도착 기록 없음 → L0
   미충족"**으로 오판정했다(상세는 아래 0-A "⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐" 절). *"문서의 기록치는 출처가
   아니라 검증 대상"*의 **역방향** 사례 — 실측 즉시 대장 반영을 발주할 것.
...
검토하게 될 경우의 함정 목록·출처는 이미 조사해 뒀다 — 아래 "7-0000000. 2026-09-19 세션이 남긴
함정" 절(조사 기록이며 착수 계획이 아님, 그 절 자체도 결과 반영 추신을 달아 뒀다).
...
## 7-00000000. 2026-09-20 세션(scribe, 기록 보강 — #211·#230·#232·#235)이 남긴 함정
...
## 7-000000000. 2026-09-20~21 세션(대장 #232 태스크①·#211 실기 검증)이 남긴 함정

> ⚠️ **번호 확인**: 바로 위 "7-00000000"(0이 하나 적다) 절이 **같은 날 먼저 쓰인 절**이다 —
...
- ⚠️ **실측이 있는데 기록이 없으면 후속 판단이 왜곡된다** — 대장 #211 사례. **전문은 바로 위
  "7-00000000" 절**(중복 방지, 규율 13 원문은 그쪽에 있다). 요지: 실측이 벌어진 순간이 아니라
