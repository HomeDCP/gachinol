...
### ⭐ 지금의 다음 1건 (2026-09-06 갱신) — S1 → S2(사용자 실행) → S3

**S0(채번·편입)는 이 세션에서 scribe가 완료했다.** 순서는 계획 §4-2가 이미 정했다 — 조율자가
바꾸지 않는 한 이 순서를 따른다.

1. **S1(⚠️ 이 세션 중 동시 진행 정황 관찰됨) — `deploy-smoke.mjs` 5xx 실패 처리(대장 #204, 규율 23
   D1)**. 선행은 S0 채번뿐(완료). **scribe 실측(2026-09-06, 같은 세션 안에서 재현 명령을 두 번
   실행)**: 대장 #204 등재 시점(첫 실행)엔 `exit=0`(PASS로 오판정)이었는데, 나중에 같은 명령을
   재실행하니 `exit=1`(FAIL — `필수 라우트 누락: /health/readiness`)로 바뀌어 있었다 —
   `git status --short`에 `infra/scripts/deploy-smoke.mjs`·`deploy-smoke.test.mjs`·
   `.github/workflows/build-images.yml`이 미커밋 수정으로 잡혀, 동시 세션이 이미 이 작업을 시작한
   것으로 보인다(대장 #204 행 비고 칸에도 이 관찰을 남겼다 — **상태 칸은 바꾸지 않았다**, 판정은
   조율자·verifier 몫). **착수 전 반드시 재확인**: 대장 #204 행의 확인 방법 칸 명령을 그 자리에서
   재실행 — `exit=1`이면 이미 진행/완료됐으니 새로 만들지 말고 verifier 위임으로 전환할 것,
   `exit=0`이면 그대로 착수. 완료 판정은 계획 §4-2 S1 상세(`routeFailures` 조건에 `status >= 500`
   추가 + `/health/readiness` 라우트 추가)와 §4-3 커밋 순서 2번.
2. **S2(사용자 실행, 코드 착수와 병렬) — C-4·C-5**. 정본은 [QUEUE.md](QUEUE.md) §C. 리드타임이 있어
   S1과 동시에 요청해야 한다(오늘 일간보고서 ①에 이미 올렸다 — 그 이후 세션은 완료 여부만
   `gh secret list`(`MONITOR_HEARTBEAT_URL` 존재 확인 — 값은 안 보임)로 재확인할 것).
3. **S3(S1 완료 + S2 완료가 머지 조건) — `monitor-freshness.mjs` + `monitor.yml`**. S2가 안 끝난 채
   머지하면 첫 스케줄 런이 시크릿 부재로 실패하는 상태를 main에 두게 된다(계획 §4-3 커밋 순서 4번이
   명시적으로 금지) — **S2 완료 확인 없이 이 커밋을 main에 올리지 말 것.**

가변 값(현재 진행 상태·정확한 착수 커밋)은 여기 적지 않는다 — 위 확인 명령으로 그 자리에서 잰다.
