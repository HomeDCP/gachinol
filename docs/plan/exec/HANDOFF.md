...
## ⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 지금의 다음 1건 (2026-09-21, scribe 갱신 — 대장 #232 태스크① 배포 완료, 태스크②~⑤·사용자 결정 4건 정리 — 이번부터 문서 최상단에 둔다 / scribe 2차(2026-09-22) — 태스크② 완료·PR #112 대기, 다음 1건=#236, 사용자 대기 8건, 오디오 실측 추가, 함정 절 재정렬 — 같은 절을 갱신한다, 규율 13 / scribe 3차(2026-09-22) — 1080p 마스터 실증 완료(대장 #232)·대장 #236 착수 게이트 해제·#239 채번 / scribe 4차(2026-09-22) — 사용자 결정 5건 확정·#240·#241 채번·PR #112·#113 머지 완료로 #139 해소·#238 차단 해제·#236 구현 중 / scribe 5차(2026-09-23) — 대장 #242 채번(ffmpeg 바이너리 환경차), PR #112~#115 머지 완료로 #236 해소, #240·#241 구현완료·PR #116 CI 수리 중, 함정 2026-09-23 신설 / scribe 6차(2026-09-26) — PR #116·#117 머지 완료로 #240·#241 해소, 대장 #243 채번(배포 자동 롤백 부재 — required_reviewers 해제의 선행 조건), 다음 1건=#243)
...
구현 중이다**(선행 전부 충족, 착수 게이트 없이 진행 중).~~ → **✅ #236 해소(2026-09-23, 아래
참조)**.

**지금의 다음 1건 = 대장 #243(배포 자동 롤백 구현, 사용자 결정 — required_reviewers 해제의
선행 조건, 2026-09-26 채번).** 처방은 planner 판정 대상이라 scribe는 등재만 한다. 상세는 아래
참조.
...
판정 대상). 상세는 PIVOT-PLAN 대장 #242 행. **진행 상황(2026-09-26): 변화 없음, 처방 미정 유지.**

**진행 반영(2026-09-26, 조율자) — PR #116·#117 전부 머지·배포 완료.** main HEAD `0f9be3f`,
제온 실물 확인(조율자 실측 인용 — scribe는 제온 SSH 금지라 독자 재현 못함): api·media-worker
이미지 둘 다 `sha-0f9be3f`, `deploy` 잡 실제 실행(스킵 아님). `/app/dist/env.js`에
`MEDIA_MASTER_LONG_EDGE` default **1920**·`MEDIA_MASTER_SHORT_EDGE` default **1080** 실효,
구 키 `MEDIA_MASTER_HEIGHT` fail-fast 코드 존재. ⇒ **대장 #240·#241 해소.** ⚠️ **단 "세로
실촬영본 종단 실증"은 아직 없다** — 태스크①(대장 #232)이 배포 후 이틀간 미실증으로 머문 선례가
있으므로, 신규 세로 촬영본 1건이 실제로 1920 긴 변으로 산출되는 것을 확인하기 전까지는 아래
사용자 대기 목록에 남긴다.

**⭐⭐ 신규 채번 — 대장 #243(배포 후 검증 실패 시 자동 복구 장치가 없다, 사용자 결정 2026-09-26의
선행 작업).** 사용자가 머지 후 승인 게이트(`required_reviewers`)를 없애고 머지만으로 배포가
자동 진행되게 하자고 제안했다 — 사용자 원문: *"내가 머지 후 승인을 하지마는, 리뷰할 능력이
없어서 사실상 리뷰없이 승인이 되는데, 머지하기만 하면 deploy web 그리고 build images 가
자동으로 진행되도록 하면 더 편할것 같은데, 이렇게 변경할 경우 문제가 생길까?"* 조율자 실측:
`build-images.yml`의 `deploy` 잡은 `docker compose pull`+`up -d` **다음에** 검증 3종(api SHA
대조·라우트 스모크·media-worker/ai-worker SHA 대조)을 돈다 — **검증 실패 시점엔 이미 새 이미지가
서빙 중**이다. 복구는 수동이고 2026-09-12에 이미 한 번 사고를 냈다(`IMAGE_TAG` 누락으로 옛
`latest` 롤백, HANDOFF 함정 절 참조). ⇒ 사람 승인 게이트를 없애면 이 경로가 더 자주·더 늦게·
무인 상태로 발동한다 — **게이트 제거의 선행 조건으로 채번**. 완화 요인(과장 금지): 배포에 DB
마이그레이션 단계가 없어 데이터 파괴 경로 자체가 없고, `preflight`가 승인 요청 전에 파괴적
마이그레이션 검사·시크릿 fail-closed를 이미 수행한다 — "데이터 유실 위험"이 아니라 "복구
자동화 부재"다. 처방 미정(planner 발주됨, scribe는 판정하지 않는다). 상세는 PIVOT-PLAN 대장
#243 행.

**사용자 결정·요청 대기 ~~8건~~ → ~~2건~~ → 실질 4건**(scribe 6차 갱신, 2026-09-26 — 신규 ⓙⓚ
2건 추가, 기존 목록은 지우지 않는다, 규율 13):
...
- ⓙ **신규(2026-09-26) — 세로 실촬영본 종단 실증**: 대장 #240·#241은 해소 표기됐으나(위 참조)
  세로 방향 신규 촬영본이 실제로 1920 긴 변으로 산출되는 것은 아직 확인된 바 없다. 태스크①
  (대장 #232)이 배포 후 이틀간 미실증으로 머문 선례가 있어 별도 항목으로 유지한다.
- ⓚ **신규(2026-09-26, 사용자 실행) — Environment `required_reviewers` 해제**: 사용자가 머지 후
  자동 배포 전환을 요청했고 조율자가 순서를 승인했다 — **대장 #243(자동 롤백 구현)이 머지된
  뒤에** 해제를 실행한다(먼저 해제하면 그 사이가 보호 공백이 된다). 지금은 아직 실행 시점이
  아니다.
...
- ⭐ **신규(2026-09-22 채번, scribe)**: **#236**(태스크③, 위 표 — ✅ 해소) ·
  **#237**(태스크④, 재생성 소스 폴백, 미착수) · **#238**(태스크⑤, 조건 확정·착수 가능·발동은
  ...
  네이티브 규격 미달 — ✅ 해소) · **#241**(마스터 오디오 규격 상향 — ✅ 해소).
- ⭐ **신규(2026-09-23 채번, scribe)**: **#242**(ffmpeg 바이너리 환경차 — 조율자 실측,
  처방 미정, 변화 없음).
- ⭐ **신규(2026-09-26 채번, scribe)**: **#243**(배포 자동 롤백 부재 — required_reviewers
  해제의 선행 조건, 처방 미정, 위 참조).
...
grep -n "^| 139 \|^| 211 \|^| 230 \|^| 232 \|^| 233 \|^| 234 \|^| 235 \|^| 236 \|^| 237 \|^| 238 \|^| 239 \|^| 240 \|^| 241 \|^| 242 \|^| 243 " docs/plan/PIVOT-PLAN.md
node infra/scripts/daejang-recheck.mjs --only 139,211,230,232,233,234,235,236,237,238,239,240,241,242,243
```
