...
| T-W4-02 **[SOLO]** | 08§A W4 | 조율자(테크리드 사상) | `.github/workflows/ci.yml`(수정, T-W2-17이 등재한 비필수 잡과 동일 파일 — 순차 편집) | **역할 명확화(D8-1, EVAL-ROUND-8)**: 웹 E2E(Playwright) 잡 **신설**은 T-W2-17(W2)이 이미 수행 — 본 태스크는 그 잡을 `required`(CI 필수 게이트)로 **승격만** 한다(잡 존재 W2부터 · 필수화 W4, 상보 관계) | 08§A W4 "웹 E2E를 CI 필수 게이트로" | qa-verifier + 루트 회귀(QA 리드 실행·조율자 결과 수신 — E5§A 게이트③ 인용, G3 대상: CI 설정) |
