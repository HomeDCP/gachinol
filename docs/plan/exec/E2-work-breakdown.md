...
| T-W1-09 | 02§E-17, 06§F-6 | FE 리드 | **예상 경로(Z-8 반영)**: `apps/subscriber/app/support.tsx`(신규, expo-router 규약, `tel:` 링크·카톡 채널 링크·FAQ 정적) + `apps/subscriber/src/support/faq-data.ts`(신규, FAQ 정적 데이터 분리) = **총 2파일**(EVAL-ROUND-8 R-7 확정 — 舊 "1~2"를 E3 Wave 9 셀(2파일)과 등식 성립하도록 확정 수로 정정). **`app/_layout.tsx` 옵션 필요 여부 판정(D12-1, EVAL-ROUND-12 M-7)**: 리포 실측(`cat apps/subscriber/app/_layout.tsx`) 결과 기존 명시 등록 라우트(`watch/[id]`·`live/[id]`)는 전부 **동적 세그먼트 + 외부 딥링크(카톡 공유)가 유일 진입 경로인 콘텐츠 상세 화면**이라 Stack 기본값(`headerShown:false`)을 오버라이드해 제목·헤더를 노출한다. `support.tsx`는 **정적 페이지**이고 지사목록/설정 등 **앱 내부 탭 화면에서 링크로 진입**하는 것이 주 경로라 이 오버라이드 이유가 성립하지 않는다 — **판정: 옵션 불요, `app/_layout.tsx`는 본 태스크 소유권에 불포함**(Wave 9 편성 유지) | 웹앱 "문의하기" 라우트 | 02§E-17 "웹앱 '문의하기' 라우트(`tel:` 링크·카톡 채널 링크·FAQ, 정적) — 06 §F-6 수신" | qa-verifier |
