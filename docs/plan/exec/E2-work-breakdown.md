...
| T-W1-03 | 02§E-4(3항목 결합: export 스모크·hls.js·고정 OG, 동일 앱·동일 담당이라 결합) | FE 리드 | **예상 경로(EVAL-ROUND-5 영역3 감점1·U-9 정정)**: `apps/subscriber/src/live/*`(hls.js 어댑터 신규 2) + `apps/subscriber/app/live/[id].tsx`(**라이브 재생 컴포넌트 수정 1**, hls.js 통합) + `apps/subscriber/app/(tabs)/index.tsx`+`apps/subscriber/app/(tabs)/stations.tsx`(**홈·지사목록 라우트 OG 메타 2**) + `apps/subscriber/package.json`(수정, hls.js 신규 의존성 추가 1 — **D6-1**) = **총 6파일**(EVAL-ROUND-9 S-13 확정 — 舊 "hls.js 어댑터 신규 1~2" 범위 표기를 총계 6파일과 열거 합이 정확히 맞도록 "2"로 정정) | `expo export --platform web` 스모크 통과 + hls.js 어댑터(Chrome/Edge/삼성인터넷 MSE) + 고정 페이지 정적 OG 메타 **+ 02§B 표 행 추가**(신규 의존성 hls.js, D6-1) | 02§E-4 "구독자 앱 `expo export --platform web` 스모크 → 전 화면 웹 렌더 확인 → hls.js 어댑터 → 고정 페이지(홈·지사 목록) 정적 OG 메타" | qa-verifier + 루트 회귀(QA 리드 실행·조율자 결과 수신 — E5§A 게이트③ 인용, D8-4: `pnpm-lock.yaml` 준-공용 자산 5종 회귀 규칙 대상, EVAL-ROUND-8 R-14) |
...
| T-W2-08 **[SOLO]** | 02§E-13(백엔드분)·02§D-T9·**03§C-5(검수 게이트 서버측 강제, EVAL-ROUND-13 D13-2/K-8)** | BE 리드 | `services/api/prisma/schema.prisma`(마이그레이션: `ContentOrigin` 확장, 1) + `services/api/src/resident-links/*`(신규 모듈, **컨트롤러 1 + 서비스 1 = 2파일**(검수 승인 전 `processing` 진입 차단 전이 가드 로직 포함, D13-2), 4엔드포인트는 동일 컨트롤러 내 — EVAL-ROUND-4 영역3 감점1·V-11 확정, go-link·telemetry 자매 모듈과 동일 산정 관례) + **`services/api/src/resident-links/resident-links.service.spec.ts`(신규 1, 검수 게이트 단위 테스트 1건 — D13-2 확정 문안)** + `services/api/src/app.module.ts`(준-공용 자산, 1줄 등록 — D4, 1) = **총 5파일**(舊 4파일에서 단위 테스트 1건 추가, 버킷 불변 M) | `POST /v1/resident-links`·`GET /v1/resident-links/:token`·`POST /v1/resident-links/:token/uploads`·`POST /v1/resident-links/:token/uploads/:uploadId/complete` — 72시간 만료·링크당 5건·건당 500MB·IP시간당10회 제한 **+ 주민 링크 업로드 콘텐츠의 지사 담당자 검수 승인 전 정식 파이프라인(`processing`) 진입 차단 전이 가드 + 단위 테스트 1건**(D13-2 확정 — 03§C-5 "검수 게이트: 지사 담당자 승인 필수, 미승인 콘텐츠는 정식 파이프라인 미진입"의 서버측 강제 유일 소유자. **T-W2-09(FE)는 이 서버측 강제의 소유 밖**이다 — T-W2-09는 검수 게이트 진입 UI만 소유하며 `services/api`를 만질 수 없음(E1 §A FE 리드 금지 칸), 해당 태스크 행 참조) | 02§D-T9 "72시간 만료·링크당 5건·건당 500MB·동일 IP 시간당 업로드 시도 10회 초과 시 차단 ... `ContentOrigin` 확장 마이그레이션". 03§C-5 "검수 게이트: 지사 담당자 승인 필수, 미승인 콘텐츠는 정식 파이프라인 미진입"(D13-2 결박 — EVAL-ROUND-13 K-8: 舊 이 불변식의 서버측 강제를 소유한 태스크가 없었다) | qa-verifier + 루트 회귀(QA 리드 실행·조율자 결과 수신 — E5§A 게이트③ 인용, G3 대상: Prisma 스키마) |
...
**W2 코드 태스크: 18건.**(EVAL-ROUND-8 D8-1 — T-W2-17 신설로 17→18) (W2 실기기 완주 DoD는 §E T-NC-07 참조)
...
| 5 | W2 롤업 — T-W2-01~17 + T-NC-07(개별 태스크 인용, EVAL-ROUND-10 P-3 정정 — 舊 "T-W2-01~16b"는 EVAL-ROUND-8 D8-1의 T-W2-17 신설 미반영 stale 표기였다. 같은 문서 §C "W2 코드 태스크: 18건"·§F-1 라벨 9("...+T-W2-17")·T-NC-07 게이트 열("T-W2-01~17")은 이미 갱신돼 있었다) | 롤업 |
...
일치). **총 태스크 수 재계산(EVAL-ROUND-8 반영 — DD1 T-W1-07 분할 42→43, DD5 T-NC 13→15, D6-3 T-NC 15→19,
D8-1 T-W2-17 신설 43→44)**: 코드 **44건**
(W0 6·W1 13·W2 18·W3 4·W4 3) + 트리거대기 3건 + 코드외 **19건**(T-NC-16~19 신설 반영, D6-3) = **66건**.
