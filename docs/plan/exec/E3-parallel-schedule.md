...
T-W1-03 ─ T-W1-07a(**DD1 — 재생 컴포넌트 호출 지점 배선은 T-W1-03 완료 후에만 안전, Wave 8에서 제거해 Wave 12로 이동**)
T-W1-03 ─ T-NC-06(**EVAL-ROUND-5 영역4 감점1·U-10 — 08§A W1 DoD② "상세 화면이 실제로 렌더됨을 확인"은 웹 빌드
산출물(`expo export --platform web` 스모크·전 화면 웹 렌더)이 있어야 판정 가능하며 그 산출물은 T-W1-03(Wave 8)뿐이다.
...
| **8** | T-W1-07b, T-W1-03, T-W1-04, T-W2-10 | **구독자 재생 컴포넌트는 T-W1-03 단독 소유 — 계측 배선은 T-W1-07a로 분리해 Wave 12로 이동(DD1, EVAL-ROUND-2 영역4 감점1·Y-13 정정)**: `apps/reporter` 이벤트훅+업로드위저드/모드선택 호출배선(T-W1-07b) vs `apps/subscriber/app/live/[id].tsx`(T-W1-03, 재생 컴포넌트 수정)+`apps/subscriber/app/(tabs)/{index,stations}.tsx`(OG메타) vs `apps/subscriber/src/pwa/register-service-worker.ts`+`apps/subscriber/public/sw.js`(신규)+**`apps/subscriber/app/_layout.tsx`(T-W1-04, SW 등록 호출 — 수정)**(EVAL-ROUND-5 영역4 감점2·U-11 정정: 舊 셀은 T-W1-04를 "SW/workbox(신규)"로만 축약해 `_layout.tsx` 수정을 누락했었다. E2 파일 목록 전건 대사 결과 T-W1-03의 소유 파일(`live/[id].tsx`·`(tabs)/index.tsx`·`(tabs)/stations.tsx`)과 T-W1-04의 소유 파일(`_layout.tsx`·`pwa/*`·`public/sw.js`)은 **동일 앱이지만 파일 집합이 겹치지 않는다** — 배타 실제 성립, 근거 서술만 불완전했었다) vs `services/api/src/media/*`(공개렌디션, api 모듈 신설 아님 — 기존 media 모듈 확장이라 `app.module.ts` 무편집) — 4개 상호 배타, qa-verifier 1슬롯 확보 | **M+M+S+S**(T-W1-07b=4파일 M·T-W1-03=5파일 M·T-W1-04=3파일 S·T-W2-10=3파일 S — Z-10 전건 재판정, 舊 M×4는 버킷 정의 미적용) | Wave 5(T-W1-03은 T-W1-02 후) |
...
| **16** | T-W3-01 **[SOLO]** | 단독 — Prisma 마이그레이션(`PushSubscription`) + `app.module.ts` 등록 | M | **Wave 12(W1 종료) + T-NC-08(스토어 계정 개설) 완료**(08§A W3 선행조건 원문 그대로 — EVAL-ROUND-4 영역1 감점4·V-4 정정, D4-3 확정. 舊 "08§A W3 선행조건은 W1만 요구"는 정본이 명시한 "W1 + 스토어 계정 개설 완료" 결합 조건을 W1 단독으로 오인용한 것이었다. 단 T-NC-08은 사용자 의존 외부 리드타임이라 **코드 선행 준비(Wave 16·17a·17b)는 §F G9 우회 경로와 동형으로 로컬 검증 범위 내 가능하며, 실제 하드 차단은 T-NC-10(스토어 제출 행위)뿐**이다 — 08§D 승인 지연 완화책 준용) |
...
**정상 웨이브 코드 태스크 합계(실행 재검산, D4-5 웨이브 재편 반영 19→20라벨, Q1 원칙 — 실행 명령·출력 인용)**:
...
(총 출현 43 = 고유 43 → 중복 0건. 웨이브 **행**은 D4-5 재편으로 19→**20**(17a·17b 분리, 신규 태스크 추가는
아님). "태스크" 열만 추출 — `선행 웨이브` 열의 참조성 재언급은 별도 열이라 이 카운트에 섞이지 않는다.)
웨이브별 분포: 1(1)+2(4)+3(1)+4(4)+5(4)+6(1)+7(1)+8(4)+9(4)+10(4)+11(1)+12(3)+13(2)+14(1)+15(1)+16(1)+**17a(1)+
17b(2)**+18(2)+19(1) = **43건**(E2 §C W0~W4 코드 태스크 합계 6+13+17+4+3=43와 일치). 구현 동시 배정이 4건을
초과하는 웨이브 **0건**(모든 웨이브 태스크 열 ≤4, D3 편성 규칙 위반 없음).

## D. 코드 외/트리거 태스크의 체크포인트 배치 (게이트 트랙)

E2 §D(트리거 대기 코드 3건)·§E(코드 외 **15건**, T-NC-13·14·15 포함 — DD5)는 동시성 상한이 적용되는 서브에이전트 웨이브가
...
| T-NC-04(어르신 패널 1차) | **Wave 5(T-W1-01+T-W1-02 완료 = `packages/ui` 토큰 게이트 완료) 직후 ~ Wave 8(export 스모크) 구간**(D4-4 확정 문안, EVAL-ROUND-4 영역1 감점5·V-5 정정 — 舊 "Wave 3(토큰 게이트)만으로 개방"은 02§E-1 "스키마 반영만으로는 이 게이트가 닫히지 않는다"·08§A 시간축 매핑 U3 "토큰 게이트 완료 → 패널 R1 순서 확정"과 어긋난 오인용이었다. 08§A 시간축 매핑표 "§E 1번 완료 직후"의 "1번"은 02§E-1의 완료 정의 2단(스키마+소비전환) 전체를 가리킨다) | 08§A 시간축 매핑표 "W1 진행 중(§E 1번 완료 직후 ~ 4번 사이)" | 5만 | 없음(비차단, 정보 수집성) |
...
| T-NC-06(go. 링크 캡처) | **Wave 8 종료 후**(EVAL-ROUND-5 영역4 감점1·U-10 정정 — go. 라우트·nginx 동적라우트는 Wave 4에 이미 완료돼 캡처 "준비"는 선행 가능하나, 08§A W1 DoD②가 요구하는 "탭 후 리다이렉트→상세 화면 실제 렌더 확인" **판정**은 웹 빌드 산출물(T-W1-03, Wave 8)이 있어야 성립한다. 舊 "Wave 4 종료 후, Wave 8과 병행 가능"은 §A 신설 엣지(`T-W1-03 ─ T-NC-06`)와 모순이었다) | 08§A W1 DoD② | 5만 | **Wave 12** 종료 보고(W1 DoD 판정)의 근거 자료(DD3, Z-9 정정) |
...
## G. E2 ↔ E3 정합 자체 검산 (DD1 재검산 — 43건, 실행 grep 인용)

**검산 방법**: E2의 전 태스크 ID(61건: 정상 웨이브 코드 43 + 트리거 대기 3 + 코드 외 15)를 위 §C 웨이브 표 +
...
것은 D4-5의 웨이브 라벨 분리(17a/17b)를 매칭하기 위함 — EVAL-ROUND-4 영역4 감점1·V-7 반영.)

| 구간 | E2 태스크 수 | E3 배정 위치 | 중복/누락 |
|---|---|---|---|
| W0 코드(T-W0-01~06) | 6 | Wave 1(1)+Wave 2(4)+Wave 4(1, T-W0-06) | 0/0 |
| W1 코드(T-W1-01~11b, DD1 분할 반영) | 13 | Wave 3(01)+Wave 4(05·06)+Wave 5(08·02)+Wave 8(07b·03·04)+Wave 10(11a)+Wave 11(11b)+Wave 9(09·10)+Wave 12(07a) | **13건 전건 배정, 중복 0**(01·05·06·08·02·07b·03·04·11a·09·10·11b·07a = 13개 고유 ID) |
| W2 코드(T-W2-01~16b) | 17 | Wave4(01)·Wave5(02·04)·Wave6(08)·Wave7(13)·Wave8(10)·Wave9(09·03)·Wave10(11·12·15)·Wave12(14·16a)·Wave13(05·16b)·Wave14(06)·Wave15(07) | **17건 전건 배정, 중복 0** |
...
| 코드 외(T-NC-01~15, T-NC-14·15 신설 반영) | 15 | §D 체크포인트 표 15행 전건 | 0/0 |
| **합계** | **61** | **43(웨이브 20라벨)+3(트리거 규칙)+15(체크포인트)=61** | **전건 배정 — 누락 0건, 중복 0건**(DD1 분할로 42→43, DD5 T-NC 신설로 13→15, D4-5 웨이브 라벨 19→20, 총 58→61 태스크 불변) |
...
| # | 발주처 | 수신처 | 요청 내용 | 상태 |
|---|---|---|---|---|
| 1 | E3 §C(예상 소요 열) | **E4**(토큰·시간 예산) | 웨이브 **20개**(D4-5 재편으로 19→20 라벨, 라운드4 갱신)의 상대 크기(S/M/L/XL) 표를 절대 토큰 예산 산정의 입력으로 사용 요청 — SOLO 웨이브 7건의 병렬 슬롯 손실을 토큰 예산에 반영 요청 | → 6-11 #9 참조 |
