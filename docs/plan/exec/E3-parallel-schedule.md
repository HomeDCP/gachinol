...
**준-공용 자산 2 — 루트 `pnpm-lock.yaml`(EVAL-ROUND-6 D6-1 신설, `app.module.ts`와 동형)**: 신규 npm 의존성을
추가하는 태스크(실측 4종 — T-W1-03 hls.js·T-W1-04 Workbox·T-W1-11a Playwright·T-W3-01 web-push, E2 §G 리스크
표 인용)는 `pnpm-lock.yaml`을 공유 편집한다 — **SOLO로 승격하지 않되 동시성 1**을 적용한다: **같은 웨이브에
신규 의존성 추가 태스크를 2건 이상 배치하지 않는다**(D6-1 확정 문안). 舊 Wave 8(T-W1-03+T-W1-04 동시 배치)이
이 규칙 위반이었으므로 아래 §C가 **Wave 8a/8b로 분리 재배치**한다(T-W3-01은 이미 SOLO라 자동 충족, T-W1-11a는
Wave 10에서 다른 의존성 추가 태스크와 겹치지 않아 자동 충족).
...
| **15** | T-W2-07 | 단독(다른 W2 태스크 전건 완료가 전제라 병렬 대상 없음) | **M(5파일**: `playwright.config.ts` 2 + 시나리오 스펙 2(관제·기자 각 1) + 스모크 스펙 1 — EVAL-ROUND-4 영역3·4 V-11 정정, E2 §C 파일 수 확정 인용) | Wave 4~14 전건(관제·기자 기능 완결 필요) — **W2 종료 웨이브** |
...
- [ ] **`pnpm-lock.yaml` 준-공용 자산 규칙(EVAL-ROUND-6 D6-1) — 신규 의존성 추가 태스크(T-W1-03·04·11a·T-W3-01) 동일 웨이브 배치 금지 재확인**(Wave 8a/8b 분리가 이 규칙의 최초 적용 사례)
...
| # | 발주처 | 수신처 | 요청 내용 | 상태 |
|---|---|---|---|---|
| 1 | E3 §C(예상 소요 열) | **E4**(토큰·시간 예산) | 웨이브 **21개**(EVAL-ROUND-6 D6-1 재편으로 20→21 라벨, Wave 8→8a/8b 분리)의 상대 크기(S/M/L/XL) 표를 절대 토큰 예산 산정의 입력으로 사용 요청 — SOLO 웨이브 7건의 병렬 슬롯 손실 + T-W1-03·T-W1-04·T-W3-01 사이징 변경(D6-1)을 토큰 예산에 반영 요청 | → 6-11 #9 참조 |
