...
다른 의존성 추가 태스크와 겹치지 않아 자동 충족, **T-W2-07은 Wave 15 단독 배정이라 D7-2 편입 후에도 위반
없음 — 아래 §C Wave 15 셀 판정 참조**).
...
| **15** | T-W2-07 | 단독(다른 W2 태스크 전건 완료가 전제라 병렬 대상 없음). **D7-2 판정(EVAL-ROUND-7 Q-9)**: T-W2-07이 `apps/{reporter,control-center}/package.json`(`@playwright/test`) 소유로 확정되며 신규 의존성 추가 태스크가 됐다 — `pnpm-lock.yaml` 준-공용 자산 동시성 1 규칙(D6-1) 위반 여부 판정 결과 **위반 없음**(Wave 15는 T-W2-07 1건뿐이라 "웨이브당 1건" 규칙이 자동 충족, 재배치 불요) | **L(7파일**: `playwright.config.ts` 2 + 시나리오 스펙 2(관제·기자 각 1) + 스모크 스펙 1 + `package.json` 2(관제·기자, D7-2 신규 의존성) — EVAL-ROUND-7 Q-7·Q-9 반영, 舊 M(5파일)에서 상향, E4 §A-6 Wave 15 재계산 후행) | Wave 4~14 전건(관제·기자 기능 완결 필요) — **W2 종료 웨이브** |
...
| **18** | T-W4-01, T-W4-03 | `CLAUDE.md`(수정) vs 신규 실측 리포트 문서 — 2개 상호 배타 | S+S | Wave 15(W2 종료), **Wave 17b**(W3 종료 — D4-5 재편 반영, 舊 "Wave 17"), **T-NC-10 스토어 심사 통과**(08§A W4 선행조건 "W1~W3") |
...
| **T-NC-18(03§B-4 1차, EVAL-ROUND-6 D6-3 신설)** | **Wave 15 종료 직전**(T-NC-07 W2 DoD 판정 직전 단계 = "첫 촬영 재개 전" 시점, D6-3 "W2 DoD 충족 직전") | 08§A 시간축 매핑표 "03§B-4 공급자(기자·주민) 사용성 검증 — 1차" | 5만 | 03 리스크 9 트리거(자막 입력 이탈 2명 이상 또는 완료율 90% 미만) 관측 — 미달 시 07 §3-3 대안 검토(본 문서 비범위), 코드 웨이브 자체는 비차단 |
| **T-NC-19(03§B-4 2차, EVAL-ROUND-6 D6-3 신설)** | Wave 15 종료(W2 DoD 판정, 첫 촬영 재개) + 4주 경과 시점, **Wave 16~18(W3 진행)과 병행 가능·비차단** | 08§A 시간축 매핑표 "03§B-4 공급자(기자·주민) 사용성 검증 — 2차" | 5만 | 없음(비차단, 정보 수집성) |
...
| T-TRIG-03(쉘 UA 감지) | 05§A-4 결정 시(전체) 또는 **Wave 17a** 이전 아무 여유 웨이브(감지 로직 선구축분만, 舊 "Wave 17") | 02§E-11 | (코드 태스크) | 없음(전체 기능은 05§A-4 미결정 시 대상 UI 자체가 없어 비차단) |
...
## G. E2 ↔ E3 정합 자체 검산 (EVAL-ROUND-6 재검산 — 65건, 실행 grep 인용)

**검산 방법**: E2의 전 태스크 ID(65건: 정상 웨이브 코드 43 + 트리거 대기 3 + 코드 외 19, D6-3 T-NC-16~19 신설
...
것은 D4-5의 웨이브 라벨 분리(17a/17b)를 매칭하기 위함 — EVAL-ROUND-4 영역4 감점1·V-7 반영. T-NC 카운트에
`\**`를 추가한 것은 EVAL-ROUND-6 신설 T-NC-16~19가 E2 §E에서 볼드 없이 등재됐음을 확인하는 과정에서, 舊 grep이
볼드 유무를 가정하지 않은 느슨한 패턴이라 신설분도 정확히 잡힘을 재확인한 것 — 패턴 자체는 라운드 5까지와 동일.)
...
| W2 코드(T-W2-01~16b) | 17 | Wave4(01)·Wave5(02·04)·Wave6(08)·Wave7(13)·**Wave8a(10)**·Wave9(09·03)·Wave10(11·12·15)·Wave12(14·16a)·Wave13(05·16b)·Wave14(06)·Wave15(07) | **17건 전건 배정, 중복 0** |
...
| **합계** | **65** | **43(웨이브 21라벨)+3(트리거 규칙)+19(체크포인트)=65** | **전건 배정 — 누락 0건, 중복 0건**(DD1 분할로 42→43, DD5 T-NC 신설로 13→15, D4-5 웨이브 라벨 19→20, **EVAL-ROUND-6: D6-3 T-NC 15→19·D6-1 웨이브 라벨 20→21**, 총 61→65 태스크) |
...
- [ ] **`pnpm-lock.yaml` 준-공용 자산 규칙(EVAL-ROUND-6 D6-1, EVAL-ROUND-7 D7-3·D7-2 갱신) — 신규 의존성 추가 태스크(T-W0-05·T-W1-03·04·11a·T-W3-01 5종 + T-W2-07) 동일 웨이브 배치 금지 재확인**(그 시점 전수, 닫힌 목록 아님 — Wave 8a/8b 분리가 최초 적용 사례, Wave 15(T-W2-07 단독)는 D7-2 편입 후에도 위반 없음 재확인 완료)
...
| # | 발주처 | 수신처 | 요청 내용 | 상태 |
|---|---|---|---|---|
| 1 | E3 §C(예상 소요 열) | **E4**(토큰·시간 예산) | 웨이브 **21개**(EVAL-ROUND-6 D6-1 재편으로 20→21 라벨, Wave 8→8a/8b 분리)의 상대 크기(S/M/L/XL) 표를 절대 토큰 예산 산정의 입력으로 사용 요청 — SOLO 웨이브 7건의 병렬 슬롯 손실 + T-W1-03·T-W1-04·T-W3-01 사이징 변경(D6-1) + **T-W2-07 Wave 15 사이징 변경(M→L, EVAL-ROUND-7 D7-2·Q-9)**을 토큰 예산에 반영 요청 | → 6-11 #9 참조 |
