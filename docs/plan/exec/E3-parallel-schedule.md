...
| **5** | T-W1-08, T-W1-02, T-W2-04, **T-W2-19**(舊 편성은 T-W2-02 — **EXEC-DECISIONS #22 ② 착수 전 교체**) | `services/api/src/telemetry/*`+`app.module.ts`(**D4 — 이 웨이브의 유일한 api 모듈 신설 태스크, Wave 4의 T-W1-05와 겹치지 않도록 분리 배치**) vs `apps/subscriber` 테마소비(11파일 **+ `app/_layout.tsx`·`src/global.css` 2 = 13**, 대장 #67 흡수분 — #22 ③) vs `apps/control-center/src/auth/*` vs **`apps/reporter/src/api/*`+`src/auth/auth-context.tsx`**(T-W2-19) — 4개 상호 배타, qa-verifier 1슬롯 확보. **舊 T-W2-02 이월 사유**: 이 열 맨 오른쪽이 규정하는 **T-NC-03 PoC 합격**(실기기 18시행, 사람 작업)이 착수 시점에 미충족이었다 — 게이트를 낮추는 대신 태스크를 옮기고, 대장 **#71**(기자 웹 인증 배선 부재, "Wave 5 편성 시 채번" 예약분)을 T-W2-19로 채번해 빈 슬롯에 투입했다. 두 소유 집합(`src/upload/*` vs `src/api/*`·`src/auth/*`)은 같은 앱 안에서도 경로가 겹치지 않아 **교체 후에도 배타 불변**이며, T-W2-02는 T-NC-03 합격 후 첫 여유 웨이브에 재배치한다(하류 T-W2-03이 Wave 9라 **Wave 9 착수 전 완료가 실질 기한**). **lockfile 갱신 태스크 0건**(D6-1 자동 충족 — **앵커 ⑯ 2단 재현, D25-1**: ① 문서 = §B "7종 전건 웨이브 배치 재검증" python 블록 출력에 라벨 `5`가 등장하지 않는다(등장하면 이 셀이 틀린 것) ② 리포 실체 = T-W1-02가 소비할 `@gachinol/ui` 선언은 선행 Wave 3의 T-W1-01이 이미 성립시켰다 — 현재 리포에는 그 선언이 없음을 `python3 -c "import json;print([k for k in json.load(open('apps/subscriber/package.json'))['dependencies'] if 'gachinol' in k])"` → `['@gachinol/shared']`로 확인) | **XL(T-W1-02, 12파일 — `theme.ts` 삭제 1+소비파일 11)**+**S×3**(T-W1-08=3파일·T-W2-04=3파일·T-W2-02=3파일 — Z-10 전건 재판정, 舊 M×3은 버킷 정의 미적용) | Wave 3(T-W1-02는 T-W1-01 필요), **T-NC-03 PoC 합격 필수**(아래 §D, EXEC-DECISIONS #2 — T-W2-02가 §E 7번 계열의 착수 지점, EVAL-ROUND-10 P-1 정정: Wave 9·16·18과 동형으로 T-NC 게이트를 선행 열에 명시 — 舊 이 열은 T-NC-03 게이트 없이 Wave 3만 적어 §C 표만 보고 편성 시 PoC 미완료 상태로 착수될 위험이 있었다) |
...
| **T-NC-13(W4 DoD 실측, EVAL-ROUND-1 영역1 감점3·X-10 신설)** | Wave 19 종료 후 | 08§A W4 DoD(**판정 명령의 정본 = E5 §C W4 행**(DD3 확정 문안)이고 E2 §E T-NC-13이 그 두 명령을 리터럴 인용한다 — **본 셀은 축약 인용이라 문자열까지 동일하지는 않다**(취지 동일, 표기는 셀 문맥에 맞춰 축약). 舊 "E2 T-NC-13과 동일 문안" 선언은 본 셀이 애초에 축약형이라 리터럴 대조로 반증됐다(EVAL-ROUND-28 G2-1(a), 앵커 ⑯ 동일 문안 규칙 미적용분 — 본 라운드에 사실에 맞게 정정). **재현**(앵커 ⑯ — 3자 공통 리터럴 = 명령① 대상 경로): `grep -c "CLAUDE.md docs/ --exclude-dir=exec" docs/plan/exec/E2-work-breakdown.md docs/plan/exec/E3-parallel-schedule.md docs/plan/exec/E5-quality-gates.md` → 3자 ≥1. **EVAL-ROUND-2 영역7 감점1·Y-5 정정**: 명령①`grep -rn "expo-env\.d\.ts\|EAS Build" CLAUDE.md docs/ --exclude-dir=exec`(정상서술 제외 단서 포함)+명령②`eas.json` 부재+워크플로/package.json EAS 잔재 0건) | 5만 | W4 "완료" 최종 선언(문서·CI 반영+네이티브 잔재 0 판정) — 본 항목이 없으면 §I 체크리스트의 "전 태스크 마감" 선언이 실측 근거 없이 이뤄진다 |
...
**라운드 28 수정에서 발생한 신규 위임**: **본 문서 발주분 없음** — G2-1(D28-1) 반영은 §D T-NC-13 셀의 "E2
T-NC-13과 동일 문안" 선언을 **축약 인용**으로 정정하고(본 셀은 애초에 축약형이라 리터럴 일치가 성립할 수 없는
형태였다), §C Wave 16 셀에 **이 셀이 W1 이원 선언의 정본**임을 부기하며 양단 `grep -c` 재현을 붙인 E3 내부
수정이다. 판정 명령의 정본은 **E5 §C W4 행**(DD3)이고 E2 §E T-NC-13이 리터럴 인용한다 — 그 E5 쪽 선언 정정은
D28-1이 **C팀에 직접 배정**했으므로 본 문서가 다시 발주하지 않는다(라운드 25·26에서 발주처를 소유자 문서 하나로
모은 판단과 같은 계열). 사이징·웨이브 편성·SOLO 건수·lockfile 8종 배치는 본 라운드에 손대지 않았다(모수 불변 —
재현은 §B python 블록과 위 §G 블록).
