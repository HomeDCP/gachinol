...
- **AI 워커는 GPU 불필요.** 비전은 프레임 샘플링(로컬 CPU). 한국어 STT는 ~~리턴제로(RTZR) API(월 $5~15)~~ →
  **2026-08-20 사용자 결정으로 로컬(whisper.cpp+Silero VAD)로 전환**(근거는 비용이 아니라 **데이터 주권**,
  [CLAUDE.md](../CLAUDE.md) §12) — 舊 RTZR 산정치는 §3 비용표에 이력으로 남기고 로컬 비용은 **재산정 필요(미실측)**.
...
[api (VM, 서울/싱가포르 리전)] ── S3 API ───┘
   ...
   ├ analysis 홉 → ai-worker(HTTP) → 로컬 STT(whisper.cpp+VAD, 舊 RTZR — 2026-08-20 데이터 주권 근거 전환)
...
| STT | ~~**RTZR API**~~(10h 무료 + ₩1,000/h, 舊 정본) → **로컬 whisper.cpp+Silero VAD**(2026-08-20 전환, 근거: 데이터 주권) | ~18h/월(舊 정본 산정 기준, 이력) | ~~$5~~ 재산정 필요(미실측) |
| 요약·태깅 | LLM API | | $2~5 |
| **소계 (라이브 제외)** | | | ~~**≈ $20~45/월**~~ **재산정 필요(미실측 — STT 구성요소가 미실측이라 합계도 미실측 구간, 위 STT 행 참고)** |
...
| CDN | **Cloudflare** | $0~20 |
| STT | ~~RTZR~~ → **로컬 whisper.cpp+Silero VAD**(2026-08-20 전환, 근거: 데이터 주권) (~132h, 舊 정본 기준) | ~~$83~~ 재산정 필요(미실측) |
| 비전 | Gemini Flash-Lite 키프레임 | $5~10 |
| 제주 릴레이(선택) | 소형 온프레미스 1대 | 1회성 HW |
| **소계 (라이브 제외)** | | ~~**≈ $215~460/월**~~ **재산정 필요(미실측 — STT 구성요소가 미실측이라 합계도 미실측 구간, 위 STT 행 참고)** |
...
### 4-C-1. 백업 — 사본 1벌에서 2벌로 (대장 #153, 2026-08-22 구현)

자체 호스팅 확정으로 **내구성 책임이 우리에게 넘어왔다**. 촬영 원본은 재취득이 불가능하고
(현장·시점 1회성, 07 §3-15 동의 체계상 재촬영도 간단치 않다) `/srv/dcpwork/minio`가 유일 사본이었다.

구현: `infra/backup/media-to-nas.sh` + systemd timer(04:00, `Persistent=true`). 절차는
[infra/backup/README.md](../../infra/backup/README.md).

**⚠️ 착수 전 확인에서 드러난 사실**: `pg-dump-to-r2.sh`(T-W0-04)는 작성돼 있었으나
**제온의 crontab·systemd timer 어디에도 등록돼 있지 않아 한 번도 돈 적이 없었다.**
스크립트의 존재는 백업의 존재가 아니다 — 배관 공백의 인프라판이다.

**⚠️ 이것은 오프사이트 백업이 아니다.** NAS(192.168.0.7)는 제온과 같은 건물이라
디스크·호스트 장애와 실수 삭제는 막지만 화재·도난·낙뢰는 막지 못한다.
진짜 오프사이트(R2 콜드)는 별건이며 계정·결제수단 결정 대기다. 그럼에도 지금 넣은 근거는
**사본 1개와 2개의 차이가 2개와 3개의 차이보다 크다**는 것.

**실측·리허설 (2026-08-22, 제온에서 bind mount로 NAS를 흉내 내 전 경로 실행)**

| 항목 | 결과 |
|---|---|
| 미디어 동기화 | **567,561,384 bytes** 전송 · 파일 수 원본 58 / 사본 58 일치 |
| PG 덤프 | `CREATE TABLE` 17건, gzip 무결성 통과 |
| **복구 리허설** | 덤프를 `restore_test` DB에 실제 복원 → **7 contents / 22 media_assets / 3 users, 원본과 완전 일치** |

**실가동 (2026-08-23) — NAS 공유 `gachinol-backup` 연결 완료**

`//192.168.0.7/gachinol-backup` 마운트(SMB 3.1.1, 2.6TB 여유) → systemd timer 등록 →
첫 실행 **567,719,832 bytes 전송(49 MB/s)** · PG 덤프 17테이블 · 종료코드 0.
다음 실행 `08-24 04:07 KST`(`RandomizedDelaySec=10m` 반영).

- **fstab에 `x-systemd.automount`를 추가**했다. `nofail`만으로는 부팅 시 NAS가 늦으면 **영영 마운트되지
  않은 채 남아 백업이 조용히 안 돈다** — 정확히 #160(스크립트는 있는데 스케줄이 없어 한 번도 안 돈 것)과
  같은 실패 유형이다. automount는 백업 시각에 접근하면 그때 붙는다.
- **`/opt/gachinol` → `/home/homedcp/gachinol` 심링크**로 유닛의 경로 가정을 실제 배포 경로에 잇는다.
- ★ **검증 지표를 `.minio.sys` 제외로 바꿨다**(첫 운영에서 드러남): MinIO 내부 메타는 수시로 생기고
  사라지는데 `--delete`를 쓰지 않아 사본에만 누적된다(실측 원본 18 / 사본 42). 전체 파일 수로 대조하면
  그 노이즈가 **실제 콘텐츠 누락을 가려버린다** — 같은 시점 콘텐츠 객체는 28/28로 정확히 일치했다.
  **검증은 우리가 지키려는 것(버킷 안 실제 객체)만 센다.**

> **root로 실행해야 한다**(실측 근거): MinIO 데이터 디렉터리가 `drwx------ root root`라
> 일반 계정 `rsync`는 **code 23**으로 죽는다. 권한을 낮추면 "부분만 백업되는데 종료 코드는 성공"이
> 되기 쉬워, 대신 쓰기 범위를 좁힌다(백업 전용 NAS 계정 + 해당 공유에만 쓰기).

> **fstab에 `nofail` 필수**: 없으면 NAS가 꺼져 있을 때 부팅이 emergency mode로 빠지고
> 헤드리스라 SSH도 안 된다(`/srv/dcpwork`가 이미 같은 이유로 달고 있다).
> **호스트명이 아니라 IP를 쓴다** — 제온의 resolv.conf가 Tailscale MagicDNS로 덮여 있어
> `HomeDCP`가 해석되지 않는다(445·139·2049는 IP로 전부 도달 확인).
...
**확정된 것** (조사 근거, §1~§5. **★ = 2026-08-17 사용자 결정으로 갱신** · **★★ = 2026-08-20 사용자 결정으로 갱신**):
- ★ 스토리지 = **제온 자체 호스팅 MinIO**(용량은 HDD 증설), 앞단 Cloudflare 무료 티어. **R2는 대기 옵션**(전환 트리거 §4-C-0)
- 라이브 = Cloudflare Stream (관리형), GPU 불요. ★★ AI STT = ~~리턴제로(RTZR)~~ → **로컬 whisper.cpp+Silero VAD**(근거는 비용이 아니라 **데이터 주권**, CLAUDE.md §12)
