...
- [ ] **W3. 쉘·PWA** — PWA 3종 → Android TWA → iOS 통합 쉘 심사(스토어 계정은 8주 전 개설 — ⚠️ 사실 정정 2026-09-01, 규율 13: 개인사업자는 Apple D-U-N-S 불요·리드타임 소멸, 대신 Google Play 개인 계정은 W3 코드 완료 후 최소 14일 필요. 상세: 07 §6 "스토어 계정 개설" 블록)
- [ ] **W4. 정리** — 네이티브 트랙 종료 선언·문서/CI 정리·웹 E2E 필수 게이트화
- 병렬 트랙: **T-AI**(실 STT·비전 주입, auto_edit — W 게이트 비의존) · 커머스 2단계(GMV 트리거 시)
  > **2026-08-17 갱신**: `auto_edit`이 **T-AI의 선택 항목에서 파이프라인 정식 단계로 승격**됐다(사용자 지시).
  > **2026-08-20 갱신(PR #54)**: 범위가 확정되고 **Phase 1이 완료**됐다 — 대장 #98 종결.
  > 범위는 3단계다: **① 기계편집**(음량 정규화·렌디션, **AI 호출 0회**) = **완료** ·
  > **② 글콘티 3지선다**(구성방향/구간/자막 문구, 맥 스튜디오 추론 노드) = **T-AI, W2 완료 후** ·
  > **③ 아카이브 B-roll 매칭** = 펀딩 후.
  > 핵심은 **auto_edit이 LLM에 의존하지 않는다**는 것 — `editPlan`이 null이면 컷 없이 기계편집만 하므로
  > AI·추론 노드가 전혀 없어도 파이프라인이 완주한다. 그래서 ①이 W 게이트와 무관하게 먼저 끝날 수 있었다.
  > STT 제공자는 **로컬 whisper.cpp+VAD로 변경**(舊 RTZR — 근거는 비용이 아니라 데이터 주권, CLAUDE.md §12).
  > 상세는 CLAUDE.md §11 "자동편집 Phase 1 완료" · 설계는 `~/.claude/plans/llm-poc-cheeky-thacker.md`.
...
- [ ] ~~SNS 다채널 송출 (YouTube → Meta → X → Threads)~~ → **YouTube 단독으로 축소**(2026-08-17 사용자 결정).
      송출 채널 = **카카오톡 + YouTube 2채널**. Meta/X/Threads 어댑터는 코드 보존·미사용. infra §5-2-1
